...
--------
1.6.2.1で更新を終了します。
スキルテーブルはフォークした上でソース中のskillUnion.jsonのURLを修正すればいじれると思います。

育成講座中は、まとめて隠し玉や連続スキル合成で追加パラメータが必要になります。
ad_idというパラメータを有効にした上で、その時ごとに設定されている数値を入れてあげてください。
(合成確認画面のソースで検索してください)
...
* 空き地戦力処理の見直し
 - 10章1～3期と11章のデータを登録
* まとめて隠し玉の小姓抜き出し処理修正
* 上位訓練で必要資源がマイナスになった場合の資源変換計算を修正
* 書状の座標っぽい文字列をリンクにする処理を修正
